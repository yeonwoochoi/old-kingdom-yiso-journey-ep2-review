...
namespace Core.Scene {
    ...
    ///   - 이전 씬 메모리 해제
    ///   - 씬 전환 시 SceneTransitionEvent 발행 → CameraSystem, SoundSystem 등 구독
    /// [타입] MonoSingleton (LoadSceneAsync 코루틴 필요)
    /// </summary>
    public class YisoSceneSystem : YisoMonoSingleton<YisoSceneSystem>, IYisoSystem {
        ...
        public void Initialize() {
            ...
            YisoLogSystem.Warn($"{SceneManager.GetActiveScene().name}이 YisoSceneType으로 파싱되지 않습니다. (일단, Login으로 설정)");
        }
        ...
        private IEnumerator LoadSceneCo(YisoSceneType next) {
            ...
            yield return YisoYieldCache.Seconds(YisoUISystem.FADE_DURATION);
