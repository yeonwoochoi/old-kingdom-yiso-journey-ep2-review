...
namespace Core.Config {
    ...
    ///   - 저장된 값이 없을 때 ConfigSO 기본값 반환 (SaveManager 초기 로드 대비)
    ...
    public class YisoConfigManager : YisoMonoSingleton<YisoConfigManager> {
        ...
        protected override void Awake() {
            ...
            if (_config == null)
                throw new InvalidOperationException($"[ConfigManager] ConfigSO를 찾을 수 없습니다.");

            _aesKey = DeriveKey(ResolveSecret());
            BuildDefaults();
            YisoLogger.Configure(_config.environment != AppEnvironment.Live);
        }
        ...
        private string ResolveSecret() {
            ...
            if (_config.environment == AppEnvironment.Live)
                throw new InvalidOperationException($"[ConfigManager] {SECRET_ENV_KEY} 환경 변수가 설정되지 않았습니다. Live 빌드에서는 필수입니다.");

            YisoLogger.Warn($"{SECRET_ENV_KEY} 미설정 - Dev 폴백 키 사용 (Live 빌드 절대 금지)");
        ...
        private bool TryDecrypt(string ciphertext, out string plaintext) {
            ...
            try {
                ...
                YisoLogger.Warn($"PlayerPrefs 복호화 실패, 기본값 사용: {e.Message}");
                return false;
            }
        }
