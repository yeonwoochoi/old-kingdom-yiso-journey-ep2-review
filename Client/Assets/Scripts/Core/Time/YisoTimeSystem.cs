...
namespace Core.Time {
    ...
    ///
    ///   - [Phase 3] 플레이타임 누적
    ///       캐릭터별 플레이타임은 TimeSystem이 누적, 저장은 SaveSystem 담당 (PlayerPrefs 직접 접근 금지)
    ///       YisoTickEvent 주기적 발행 → SaveSystem이 구독해서 자동 중간저장
    ///
    ///   - [Phase 5] 게임 시간 흐름 관리
    ///       실제 N초 = 게임 1시간으로 환산 (REAL_SECONDS_PER_GAME_HOUR 참조)
    ///       TimeChangedEvent 발행 → EnvironmentSystem 구독 (낮/밤 사이클 등)
    /// [타입] MonoBehaviour (Update, 코루틴 필요)
    /// </summary>
    public class YisoTimeSystem : YisoMonoSingleton<YisoTimeSystem>, IYisoSystem {
