...
namespace Gameplay.Health.GUI {
    public class YisoProgressBar : RunIBehaviour {
        ...
        [Title("Basic")] public string playerID;
        public Transform foregroundBar;

        public Transform
            delayedBarDecreasing; // the delayed bar that will show when moving from a value to a new, lower value

        public Transform
            delayedBarIncreasing; // the delayed bar that will show when moving from a value to a new, higher value\

        [Title("Initial Value")] [FormerlySerializedAs("StartValue"), Range(0f, 1f)]
        public float minimumBarFillValue = 0f;

        [FormerlySerializedAs("EndValue"), Range(0f, 1f)]
        public float maximumBarFillValue = 1f;

        [FormerlySerializedAs("InitialValue"), Range(0f, 1f)]
        public float initialFillValue = 1f;

        [Title("Foreground Bar")] public bool lerpForegroundBar = true;
        ...
        [Title("Event")] public UnityEvent onBump;
        ...
        [Title("Text")] public Text percentageText;
        public string textPrefix;
        public string textSuffix;
        public string textFormat = "{000}"; // ex. 12인 경우 012 이렇게 표시됨

        [Title("Progress (Readonly)")] [Range(0f, 1f)]
        public float barProgress; // Foreground Bar Progress (현재 %)
        ...
        public virtual void UpdateBar(float currentValue, float minValue, float maxValue) {
            ...
            // Remap current Value
            newPercent =
                YisoMathUtils.Remap(currentValue, minValue, maxValue, minimumBarFillValue, maximumBarFillValue);
        ...
        protected virtual float ComputeNewFill(bool lerpBar, float barSpeed, AnimationCurve barCurve, float delay,
            float lastPercent, out float normalizedTimeSpent) {
            var newFill = 0f;
        ...
        [Title("Tests")] [Range(0f, 1f)] public float debugNewTargetValue;
