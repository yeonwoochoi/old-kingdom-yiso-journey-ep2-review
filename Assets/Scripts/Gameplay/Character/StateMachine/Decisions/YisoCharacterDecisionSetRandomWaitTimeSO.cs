...
namespace Gameplay.Character.StateMachine.Decisions {
    ...
    public class YisoCharacterDecisionSetRandomWaitTimeSO: YisoCharacterDecisionSO {
        [Header("Input Key (Read)")]
        public YisoBlackboardKeySO targetTimeKey; // Action이 저장했던 그 키 (예: CurrentWaitTime)

        public override bool Decide(IYisoCharacterContext context) {
            var blackboardModu8le = context.GetModule<YisoCharacterBlackboardModule>();
            var stateModule = context.GetModule<YisoCharacterStateModule>();
            
            if (blackboardModu8le == null || stateModule == null) return false;

            // 저장된 목표 시간을 가져옴
            var targetDuration = blackboardModu8le.GetFloat(targetTimeKey);
